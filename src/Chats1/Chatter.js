      ...
      >
        ...
        >
          ...
          >
            ...
            />
            {!this.props.message.author ? null : this.props.message.author
                .photoThumbnail ? (
              <img
                alt="error"
                    ...
                    .then((docs) => {
                      docs.forEach((doc) => {
                        if (doc.exists) {
                          ...
                          updateDoc(doc(firestore, "chats", doc.id), {
                            threadId,
                            recipients: arrayUnion(this.state.ask.id)
                          });
                        }
                      });
                    })
            ...
            >
              {this.props.entityType !== "users" && (
                ...
                >
                  ...
                  {this.props.thisentity &&
                    new Date(
                      this.props.thisentity.eventDate.seconds * 1000
                    ).toLocaleDateString()}
                </div>
              )}
...
class CheckProfiler extends React.Component {
  render() {
    ...
    var thiscommunity = this.props.thisentity
      ? this.props.thisentity.community
      : null;
      ...
      >
        ...
        >
          ...
          <textarea
            ...
            style={{
              height:
                this.state.newMessage !== "" ? this.state.heightOfText : "36px",
              maxHeight: "90vh",
              display: "flex",
              position: "relative",
              padding: "10px",
      ...
      >
        {
          ...
          !this.props.topchats.includes(message.id) ? (
            <MessageClean
              noteList={noteList}
              noteTitles={noteTitles}
              parent={this.props.parent}
    ...
    if (
      ...
      var recipients =
        this.props.recipients.constructor === Array
          ...
          : [this.props.recipients];
      let topchats = [];
      var recentChatters = this.props.recentChats.sort((a, b) => {
        return b.time.seconds - a.time.seconds;
      });
      recipients.map((recipient) => {
        var topchat =
          recentChatters.find(({ authorId }) => authorId === recipient) ||
          false;
        topchats.push(topchat.id);
        if (
          recentChatters.length > 0 &&
          topchats.length === this.props.recipients.length
        ) {
          this.setState({ recentChatters, topchats });
        }
        return topchats;
      });
    }
    if (
      this.state.recentChatters &&
      this.state.recentChatters.length > 0 &&
      this.state.lastTopchats !== this.state.topchats
    ) {
      var datethis = this.state.recentChatters[0].time.seconds * 1000;
      var goo = new Date(datethis).toLocaleString();
      this.setState({
        lastTopchats: this.state.topchats,
        creationDate: goo
      });
    }
  };
